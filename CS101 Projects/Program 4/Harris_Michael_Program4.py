...
def acesHigh(values):
    ace_high_dict = {'T': 10, 'J': 11, 'Q': 12, 'K': 13, 'A': 14}
    new_list = []
    for num in values:
        if num.isdigit():
            new_list += int(num),
...
def acesLow(values):
    ace_high_dict = {'T': 10, 'J': 11, 'Q': 12, 'K': 13, 'A': 1}
    new_list = []
    for num in values:
        if num.isdigit():
            new_list += int(num),
...
def genDeck():
    ...
    for value in rank:
        for value2 in suit:
            #Combine all the suits and ranks into tuples and add to the deck
            deck.append((value, value2))
...
while user_interest:    
    ...
    winList = [Player1[1], Player2[1], Player3[1], Player4[1], Player5[1]]
    winRank = max(winList)
